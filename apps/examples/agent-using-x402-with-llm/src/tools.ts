...
async function handleDeposit(amount: number, tokenSymbol: string): Promise<string> {
...
export async function executeTool(name: string, input: Record<string, unknown>): Promise<string> {
	try {
		switch (name) {
			...
			case "deposit":
				return await handleDeposit(input.amount as number, input.token_symbol as string);
			case "withdraw":
